...
function Join() {
  ...
  return (
    <Container component='main' maxWidth='xs'>
      <Box sx={{display: 'flex', flexDirection: 'column', alingItems: 'center', justifyContent: 'center', height: '100vh'}}>
        ...
        <Box component='form' noValidate onSubmit={handleSubmit} sx={{mt:3}}>
          <Grid container spacing={2}>
            <Grid item xs={12}>
                <TextField name='name' required fullWidth label='닉네임' autoFocus autoComplete='off' />
            </Grid>
            <Grid item xs={12}>
                <TextField name='email' required fullWidth label='이메일 주소' autoComplete='off' />
            </Grid>
            <Grid item xs={12}>
                <TextField name='password' required fullWidth label='비밀번호 6자 이상' type='password' />
            </Grid>
            <Grid item xs={12}>
          ...
          <LoadingButton type='submit' fullWidth variant='contained' color='secondary' loading={loading} sx={{mt:3, mb:2}}>
            회원가입
          </LoadingButton>
